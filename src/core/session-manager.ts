        ...
        if (msg.type === 'event' && msg.event.type === 'status' && msg.event.status === 'idle') {
          ...
          idleTimer = setTimeout(() => {
            ...
            const finalize = (resolvedId: string) => {
              log({ level: 'debug', source: 'dispatch', summary: `Finalize: textLen=${text.length}, hasStructured=${structured !== undefined}, entryTypes=[${entryTypes.join(', ')}] (parent: ${parentSessionId})` });
              if (text || structured !== undefined || entryTypes.length > 1) {
                ...
                parts.push(`entries: [${entryTypes.join(', ')}]`);
                if (contentSummaries.length > 0) parts.push(`content: ${contentSummaries.join(', ')}`);
                log({ level: 'debug', source: 'dispatch', summary: `Null result: no text, no structured data, ≤1 entry (parent: ${parentSessionId}) — ${parts.join(' | ')}` });
                log({ level: 'warn', source: 'child-session', summary: `Turn completed with empty response (parent: ${parentSessionId}) — ${parts.join(' | ')}` });
                cleanup(/* force */ true);
                resolve(null);
              }
            };
      ...
      send(msg) {
        ...
        if (msg.type === 'event' && msg.event.type === 'status' && msg.event.status === 'idle') {
          ...
          idleTimer = setTimeout(() => {
            if (settled) return;
            settled = true;
            log({ level: 'debug', source: 'dispatch', summary: `Resume finalize: textLen=${text.length}, hasStructured=${structured !== undefined}, entryTypes=[${entryTypes.join(', ')}] (session: ${sessionId.slice(0, 12)}…)` });
            cleanup();
            if (text || structured !== undefined || entryTypes.length > 1) {
              resolve({ sessionId, text, structured });
            } else {
              log({ level: 'debug', source: 'dispatch', summary: `Resume null result: no text, no structured data, ≤1 entry (session: ${sessionId.slice(0, 12)}…)${lastError ? `, error: ${lastError}` : ''}` });
              if (lastError) log({ level: 'warn', source: 'resume-child', summary: `Empty response with error: ${lastError}` });
              resolve(null);
            }
          }, IDLE_SETTLE_MS);
        }
